	...
	Meteor.methods({
		...
		addMedication: function (data) {
			...
			if (data.conflictData) {
				...
				if (data.conflictData.medicationConflicts) {
			...
			if (Meteor.user().profile.type === 'patient') {
				var medList = Meteor.users.findOne({"_id": Meteor.userId()}).profile.medications || [];
				medList.push(medicineObj);
				var userProfile = Meteor.users.findOne({"_id": Meteor.userId()}).profile;
				userProfile.medications = medList;
				Meteor.users.update({"_id": Meteor.userId()}, {$set: {"profile": userProfile}});
			} else {
				medicineObj.prescribedBy = Meteor.user().profile.name;
				...
				if (recList) {
					...
					var userProfile = Meteor.users.findOne({"profile.userNumber": data.patientNumber}).profile;
					userProfile.recommendations = recList;
					Meteor.users.update({"profile.userNumber": data.patientNumber}, {$set: {"profile": userProfile}});
				};
			};

			return medicineObj._id;
		},
		...
		updateMedicine: function(data) {
			if (Meteor.user().profile.type === 'patient') {
				...
				if (idx !== -1) {
					...
					Meteor.users.update({"_id": Meteor.userId()}, {$set: {"profile.medications": medList}});
				};
			} else {
				var idx = -1;
				...
				if (idx !== -1) {
					...
					Meteor.users.update({"profile.userNumber": data.patientNumber}, {$set: {"profile.recommendations": recList}});
				};
			};
		}
	});
